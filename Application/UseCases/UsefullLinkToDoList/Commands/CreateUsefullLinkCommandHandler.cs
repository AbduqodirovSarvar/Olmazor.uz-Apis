...
namespace Application.UseCases.UsefullLinkToDoList.Commands
{
    public class CreateUsefullLinkCommandHandler(
        ...
        ) : IRequestHandler<CreateUsefullLinkCommand, UsefulLink>
    {
        private readonly IAppDbContext _appDbContext = appDbContext;
        private readonly IMapper _mapper = mapper;
        private readonly IFileService _fileService = fileService;

        public async Task<UsefulLink> Handle(CreateUsefullLinkCommand request, CancellationToken cancellationToken)
        {
            var usefullLink = _mapper.Map<UsefulLink>(request);
